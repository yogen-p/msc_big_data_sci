...
conv1 = tf.nn.relu(tf.nn.conv2d(inputs, weight['w1'], strides=[1,1,1,1], padding='VALID') + bias['b1'])
...
# conv3 layer with biases and NO relu: 1 filter with size 5 x 5
conv3 = tf.nn.conv2d(conv2, weight['w3'], strides=[1,1,1,1], padding='VALID') + bias['b3']
...
for i in range(filter_w1):
    sub_image = f.add_subplot(8, 8, i+1)
    sub_image.imshow(weight1[:, :, 0, i] ,interpolation = None, cmap = 'gray')
plt.show()
f.savefig('weight_subplot.jpg')


plt.imshow(weight3[:, :, 0, 0], interpolation=None, cmap='gray')
plt.show()
...
for key in weight.keys():
    sess.run(weight[key].assign(model[key]))
    
for key in bias.keys():
    sess.run(bias[key].assign(model[key]))

    
"""
Read the test image
"""
blurred, groundtruth = preprocess("butterfly_GT.bmp")
...
input_ = np.expand_dims(np.expand_dims(blurred, axis =0), axis=-1)
...
print("-"*25)
print("Saving SR image, blurred image and groundtruth image ")
scipy.misc.imsave('SR.jpg',output_[0,:,:,0])
scipy.misc.imsave('groundtruth.jpg',groundtruth)
scipy.misc.imsave('blurred.jpg',blurred)
print("-"*25)
print("Groundtruth Shape",groundtruth.shape)
print("Blurred Shape",blurred.shape)
print("SR Shape",output_.shape)

groundtruth_cropped = groundtruth[6:-6,6:-6] # Ccropping the image SR image size
blurred_cropped = blurred[6:-6,6:-6] # Cropping the image to size of SR image
sr_float = output_[0,:,:,0].astype(np.float64)  # To numpy float 64
print("-"*25)
print("Calculating PSNR values on groundtruth vs blurred_image and groundtruth vs SR image")
psnr_sr_br = skimage.measure.compare_psnr(groundtruth_cropped,blurred_cropped)
psnr_sr_gt = skimage.measure.compare_psnr(groundtruth_cropped,sr_float)
print("-"*25)
